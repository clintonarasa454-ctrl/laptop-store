...
export function getOrderConfirmationEmailHtml(opts: CommonEmailOptions & { shippingFullName: string; orderNumber: string; cartData: any[]; subtotal: number; shippingCost: number; total: number; storeCurrency: string; customMessage?: string; }) {
  const { storeName, primaryColor = "#3b82f6", contactEmail, storePhone, shippingFullName, orderNumber, cartData, subtotal, shippingCost, total, storeCurrency, customMessage } = opts;
  const logoHtml = getHeaderHtml(storeName, opts.logoUrl);
...
export function getShippingNotificationEmailHtml(opts: CommonEmailOptions & { shippingFullName: string; orderNumber: string; trackingNumber?: string; trackLink: string; customMessage?: string; }) {
  const { storeName, primaryColor = "#3b82f6", contactEmail, storePhone, shippingFullName, orderNumber, trackingNumber, trackLink, customMessage } = opts;
  const logoHtml = getHeaderHtml(storeName, opts.logoUrl);
  ...
  return `
    <div style="font-family: system-ui, -apple-system, sans-serif; color: #333; max-width: 600px; margin: 0 auto; padding: 30px; border: 1px solid #e5e7eb; border-radius: 12px; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1);">
      ...
      <p style="color: #4b5563;"><strong>Order Number:</strong> #${orderNumber}</p>
      ${trackingHtml}
...
export function getAbandonedCartEmailHtml(opts: CommonEmailOptions & { shippingFullName: string; orderNumber: string; total: string | number; orderLink: string; storeCurrency: string; }) {
  const { storeName, primaryColor = "#3b82f6", contactEmail, storePhone, shippingFullName, orderNumber, total, orderLink, storeCurrency } = opts;
  const logoHtml = getHeaderHtml(storeName, opts.logoUrl);
  const formatEmailPrice = (p: string | number) => new Intl.NumberFormat("en-US", { style: "currency", currency: storeCurrency }).format(typeof p === "string" ? parseFloat(p) : p);
  ...
  return `
    <div style="font-family: system-ui, -apple-system, sans-serif; color: #1f2937; max-width: 600px; margin: 0 auto; padding: 20px 24px; border: 1px solid #e5e7eb; border-radius: 8px; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.05);">
      ...
      <div style="background: #f9fafb; padding: 15px 20px; border-radius: 6px; margin: 15px 0; text-align: center;"><h3 style="margin: 0 0 5px 0; font-size: 14px; color: #374151;">Order #${orderNumber}</h3><p style="margin: 0; font-size: 14px; color: #6b7280;">Pending Amount: <strong>${formatEmailPrice(total)}</strong></p></div>
