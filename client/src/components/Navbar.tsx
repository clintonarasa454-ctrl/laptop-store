...
import { getGuestCart, clearGuestCart, formatPrice, addToGuestCart } from "@/lib/cart";
...
export default function Navbar() {
  ...
  const [chatMessages, setChatMessages] = useState<{ role: 'user' | 'assistant', content: string, products?: any[], suggestions?: string[], imageUrl?: string }[]>([]);
  ...
  const handleAddAiProductToCart = (e: React.MouseEvent, p: any) => {
    e.preventDefault();
    e.stopPropagation();
    if (isAuthenticated) {
      upsertCart.mutate({ productId: p.id, quantity: 1 });
    } else {
      addToGuestCart({
        id: p.id,
        name: p.name,
  ...
  useEffect(() => {
    if (dbHistory && dbHistory.length > 0 && chatMessages.length <= 1) {
      const formatted = dbHistory.map((h: any) => ({ role: h.role as 'user' | 'assistant', content: h.message, imageUrl: h.imageUrl })).reverse();
  ...
  useEffect(() => {
    ...
    const displayName = secondName ? `, ${secondName}` : " there";
  ...
  const { activeCategories, orderedCategories, rootCategories } = useMemo(() => {
    const active = (categories as any[]) ? (categories as any[]).filter(c => c.active !== false) : [];
    const ordered = [...active].sort((a, b) => ((a as any).order ?? 0) - ((b as any).order ?? 0));
    const root = ordered.filter(c => !(c as any).parentId);
    return { activeCategories: active, orderedCategories: ordered, rootCategories: root };
  }, [categories]);
  ...
  const { data: searchResults, isLoading: searching } = trpc.products.list.useQuery(
    { search: debouncedSearch, limit: 5, lat: geoLocation?.lat, lng: geoLocation?.lng },
    { 
  ...
  const handleDragStart = (e: React.PointerEvent<HTMLDivElement>) => {
    ...
    dragStartRef.current = {
      x: e.clientX - chatPositionRef.current.x,
      y: e.clientY - chatPositionRef.current.y
    };
  ...
  const handleDragEnd = (e: React.PointerEvent<HTMLDivElement>) => {
    ...
    sessionStorage.setItem("store_ai_position", JSON.stringify(chatPositionRef.current));
  };
  ...
  const handleAiSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if ((!chatInput.trim() && !selectedImage) || aiMutation.isPending) return;
    const userMsg = chatInput.trim() || "Analyze this image";
    setChatInput("");
    const currentImage = selectedImage;
    setSelectedImage(null);

    const newMessages: { role: 'user' | 'assistant', content: string, products?: any[], suggestions?: string[], imageUrl?: string }[] = [...chatMessages, { role: 'user', content: userMsg, imageUrl: currentImage || undefined }];
  ...
  const handleSuggestedPrompt = (prompt: string) => {
    ...
    const newMessages: { role: 'user' | 'assistant', content: string, products?: any[], suggestions?: string[], imageUrl?: string }[] = [...chatMessages, { role: 'user', content: prompt }];
    setChatMessages(newMessages);
    setDynamicSuggestions([]);
    const currentCart = isAuthenticated
      ? (cartItems || []).map(i => ({ productId: i.productId, quantity: i.quantity }))
  ...
  const renderAutocomplete = () => {
    ...
    return (
      <div className="absolute top-full left-0 right-0 mt-1 w-full bg-card border border-border rounded-lg shadow-lg overflow-hidden z-50 max-h-[60vh] overflow-y-auto">
        {searching ? (
  ...
  return (
    <>
      <header
        className={`sticky top-0 z-50 transition-all duration-500 ${scrolled ? "bg-background/80 backdrop-blur-xl shadow-sm border-b border-border" : "bg-background border-b border-transparent"
          }`}
      >
        {/* Top bar */}
                ...
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button variant="ghost" size="icon" className="rounded-full">
                      {user?.photoId ? (
                        <img src={user.photoId} alt={user.name || "User"} className="w-7 h-7 rounded-full object-cover border border-border shadow-sm" />
                      ) : (
                  ...
                  <DropdownMenuContent align="end" className="w-52">
                    ...
                    {(user?.role === "admin" || user?.role === "manager") && (
                      <>
                        ...
                        <Link href={user.role === "manager" ? "/manager" : "/admin"} className="flex items-center gap-2 cursor-pointer">
                          <Settings className="w-4 h-4" /> {user.role === "manager" ? "Manager Portal" : "Admin Panel"}
                          </Link>
                        </DropdownMenuItem>
                      </>
                    )}
        ...
        <div
          className={`fixed top-0 left-0 h-full w-[85%] max-w-sm bg-background border-r border-border z-[70] transform transition-transform duration-300 ease-in-out md:hidden flex flex-col shadow-2xl ${mobileOpen ? "translate-x-0" : "-translate-x-full"
            }`}
        >
          {/* Sidebar Header */}
                  ...
                  <div className={`max-w-[85%] sm:max-w-[75%] ${msg.role === 'user'
                    ...
                    <div className={`text-xs leading-relaxed whitespace-pre-line ${msg.role === 'user' ? 'font-medium' : 'font-normal'}`}>
                      {msg.role === 'assistant' ? (
                            ...
                            <div className="mt-4 flex flex-col gap-2 border-t border-border/50 pt-3 animate-in fade-in duration-500">
                              ...
                              <div className="flex overflow-x-auto gap-3 pb-2 snap-x [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                                {msg.products.map((p: any) => (
                                  <Link key={p.id} href={`/products/${p.slug}`} onClick={() => setAiChatOpen(false)} className="flex-shrink-0 w-36 bg-background rounded-xl p-2.5 snap-start shadow-sm border border-border/50 hover:border-[var(--brand)] hover:shadow-md transition-all group flex flex-col">
                                    ...
                                    <div className="mt-auto pt-2 flex items-center justify-between">
                                      <p className="text-[12px] font-bold text-[var(--brand)]">{formatPrice(p.price)}</p>
                                      <Button
                                        size="icon"
                                        variant="ghost"
                                        className="h-6 w-6 bg-[var(--brand)]/10 text-[var(--brand)] hover:bg-[var(--brand)] hover:text-white rounded-md shrink-0 z-10 relative"
                                        onClick={(e) => handleAddAiProductToCart(e, p)}
                                      >
                        ...
                        <>
                          {msg.imageUrl && <img src={msg.imageUrl} alt="Uploaded image" className="w-full max-w-xs h-auto rounded-lg mb-2 border border-[var(--brand)]/20 shadow-sm" />}
                          {msg.content}
                        </>
                      )}
                    </div>
            ...
            <div className="px-4 py-3 border-t border-border/50 bg-background/80 backdrop-blur-sm flex-shrink-0">
              {selectedImage && (
                <div className="relative inline-block mb-3">
                  <img src={selectedImage} alt="Preview" className="h-16 w-auto rounded-md border border-border shadow-sm object-cover" />
                  <button
                    type="button"
              ...
              <form className="flex gap-2" onSubmit={handleAiSubmit}>
                <div className="relative flex-1">
                  <input
                    ...
                    className="w-full h-10 pl-3.5 pr-16 text-xs rounded-lg border border-input/60 bg-background hover:border-input focus:outline-none focus:ring-2 focus:ring-[var(--brand)]/50 focus:border-[var(--brand)] transition-all placeholder:text-muted-foreground/50 disabled:opacity-50 disabled:cursor-not-allowed"
                  ...
                  <div className="absolute right-2.5 top-1/2 -translate-y-1/2 flex items-center gap-1">
                    <button
                      ...
                      className="transition-colors p-1 rounded hover:bg-muted/50 text-muted-foreground hover:text-foreground"
                      title="Upload Image"
                    ...
                    <button
                      ...
                      className={`transition-colors p-1 rounded hover:bg-muted/50 ${isListening ? 'text-destructive animate-pulse' : 'text-muted-foreground hover:text-foreground'} disabled:opacity-50`}
                      title="Voice Search"
                  ...
                  <input type="file" className="hidden" ref={fileInputRef} accept="image/*" onChange={handleImageUpload} disabled={aiMutation.isPending} />
                </div>
                <Button
                  type="submit"
                  size="sm"
                  ...
                  disabled={(!chatInput.trim() && !selectedImage) || aiMutation.isPending}
                >
                  Send
                </Button>
              </form>
